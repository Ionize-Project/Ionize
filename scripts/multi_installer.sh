...
# Ionize Multi-installer
# a one line clone-and-compile for Ionize:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/Ionize/Ionize/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/Ionize ]; then
            read -r -p "${1:-Ionize directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old Ionize directory" && echo
                    rm -rf "$PWD"/Ionize
                    ;;
                *)
                    _fail "Ionize directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir Ionize
        git clone -b master -q https://github.com/Ionize/Ionize Ionize   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd Ionize
    fi
}

_build_Ionize() {
    _note "Building Ionize from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/Ionize')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/Ionize')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Ionize Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'Ionize/build/src/' for the executible binaries. See 'https://github.com/Ionize/Ionize' for more project support. Cowabunga!"
